...
class PassListDecoder<T>(override val ops: DynamicOps<T>, private val input: T, level: Int) : PassDecoder<T>(ops, level) {
    ...
    init {
        debug("CREATED LIST DECODER WITH IN: $input")
    }
    ...
    override fun decodeElementIndex(descriptor: SerialDescriptor): Int {
        debug("Decoding an element.. $currentIndex")
        currentIndex += 1
    ...
    override fun <T> decodeSerializableValue(deserializer: DeserializationStrategy<T>): T {
        //return deserializer.deserialize(PassListDecoder(ops, currentValue!!, level + 1))
        println("DOOT: ${deserializer.descriptor}")
